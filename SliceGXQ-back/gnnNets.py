...
def get_gnnNets_explain(input_dim, output_dim, model_config):
    if model_config.gnn_name.lower() == 'gcn':
        gcn_model_param_names = GCNNet_explain.__init__.__code__.co_varnames
        gcn_model_params = {param_name: getattr(model_config.param, param_name)
                            for param_name in gcn_model_param_names
                            if param_name in model_config.param.keys()}
        return GCNNet_explain(input_dim=input_dim,
                      output_dim=output_dim,
                      **gcn_model_params)
    elif model_config.gnn_name.lower() == 'gin':
        gin_model_param_names = GINNet_explain.__init__.__code__.co_varnames
        gin_model_params = {param_name: getattr(model_config.param, param_name)
                            for param_name in gin_model_param_names
                            if param_name in model_config.param.keys()}
        return GINNet_explain(input_dim=input_dim,
                      output_dim=output_dim,
                      **gin_model_params)
    elif model_config.gnn_name.lower() == "gat":
        gat_model_param_names = GATNet_explain.__init__.__code__.co_varnames
        gat_model_params = {
            param_name: getattr(model_config.param, param_name)
            for param_name in gat_model_param_names
            if param_name in model_config.param.keys()
        }
        return GATNet_explain(input_dim=input_dim, output_dim=output_dim, **gat_model_params)
    else:
        raise ValueError(
            f"GNN name should be gcn " f"and {model_config.gnn_name} is not defined."
        )
...
class GCNConv(GCNConv):
    ...
    # used for graphmask
    def propagate1(self, edge_index: Adj, size: Size = None, **kwargs):
...
class GCNNet_explain(GNNBase):
    def __init__(self,
        ...
        super(GCNNet_explain, self).__init__()
        # first and last layer - dim_features and classes
        self.input_dim = input_dim
        self.output_dim = output_dim
        # GNN part
    ...
    def forward(self, *args, **kwargs):
        _, _, batch = self._argsparse(*args, **kwargs)
        # node embedding for GNN
        emb,emb_list = self.get_emb(*args, **kwargs)
        # pooling process
        x = self.readout_layer(emb, batch)
...
class GINNet_explain(GNNBase):
    def __init__(
        ...
        super(GINNet_explain, self).__init__()
        # first and last layer - dim_features and classes
        self.input_dim = input_dim
        self.output_dim = output_dim
        # GNN part
    ...
    def propagate1(self, edge_index: Adj, size: Size = None, **kwargs):
        ...
        if (isinstance(edge_index, SparseTensor) and self.fuse
            ...
            for i in range(decomposed_layers):
                ...
                coll_dict = self.__collect__(self._user_args, edge_index,
                                             size, kwargs)
...
class GATNet(GNNBase):
    def __init__(
        ...
        gnn_dropout: float = 0.0,
        gnn_emb_normalization: bool = False,
        gat_dropout: float = 0,  # dropout in gat layer
        gat_heads: int = 10,  # multi-head
        gat_hidden: int = 10,  # the hidden units for each head
        gat_concate: bool = False,  # the concatenation of the multi-head feature
        num_gat_layer: int = 3,
        readout: str = "mean",
        fc_latent_dim: Union[List[int]] = [],
        fc_dropout: float = 0.0,
        ...
        self.mlps = nn.ModuleList()
        if self.num_mlp_layers > 1:
            self.mlps.append(nn.Linear(self.dense_dim, self.mlp_hidden[0]))
            for i in range(1, self.num_mlp_layers - 1):
...
class GATNet_explain(GNNBase):
    def __init__(
        ...
        super(GATNet_explain, self).__init__()
        self.mlp_hidden = fc_latent_dim
        self.emb_normlize = gnn_emb_normalization
        self.num_gnn_layers = num_gat_layer
        self.num_mlp_layers = len(fc_latent_dim) + 1
    ...
    def forward(self, *args, **kwargs):
        ...
        emb = self.get_emb(*args, **kwargs)
        # pooling process
        x, emb_list = self.readout_layer(emb, batch)
